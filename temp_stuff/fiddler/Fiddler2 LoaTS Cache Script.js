...
class Handlers
{
    ...
    static function fpgCachePath(){return "C:/Users/USERNAME/Documents/Fiddler2/Captures/Responses";}
        
    static function fpgCacheFilePath(oSession: Session) {return (fpgCachePath() + oSession.PathAndQuery).Replace("/", "\\");}
        
    static function fpgAskCache(oSession: Session) {
        if (oSession.HostnameIs("5thplanetgames-origin.cdnetworks.net") && oSession.uriContains("/lots_live/images/")) {
        
            //FiddlerApplication.Log.LogString("###");
            //FiddlerApplication.Log.LogString("Request for " + oSession.PathAndQuery);
            
            var fp = (fpgCachePath() + oSession.PathAndQuery).Replace("/", "\\");
            
            if (fpgIsCached(oSession) == true) {
                //FiddlerApplication.Log.LogString("Using cached file " + fp);
                oSession["x-replywithfile"] = oSession.PathAndQuery.Substring(1).Replace("/", "\\");
    ...
    static function fpgIsCached(oSession: Session) {
        // Store the current request body
        var bak = oSession.RequestBody.Clone();
            
        // Check for file existence, by attepmting to load the requested file as the request body
        var exists = oSession.LoadRequestBodyFromFile(fpgCacheFilePath(oSession));
            
        // Reinstate the request
        oSession.RequestBody = bak;
        
        return exists;
        
    }
        
    static function fpgSaveToCache(oSession: Session) {
        if (oSession.HostnameIs("5thplanetgames-origin.cdnetworks.net") && oSession.uriContains("/lots_live/images/")) {
            if (oSession.responseCode == 200 && fpgIsCached(oSession) == false) {
                var fp = fpgCacheFilePath(oSession);
                //FiddlerApplication.Log.LogString("Caching file " + fp);
                oSession.SaveResponseBody(fp);
...
static function OnPeekAtResponseHeaders(oSession: Session) {
    ...
    static function OnBeforeResponse(oSession: Session) {
        ...
        fpgSaveToCache(oSession);
        
        //save grooveshark streams
        if (/grooveshark\.com/i.test(oSession.hostname)  && oSession.uriContains("/stream.php")) {
            oSession.SaveResponseBody(fpgCachePath() + "/" + oSession.SuggestedFilename);
        }
